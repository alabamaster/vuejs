...
const App = {
	data() {
		return {
			isFinish: false,
			activeStepIdx: 0, // то, что позволяет определить текущий активный шаг
			steps: [
	...
	methods: {
		getBack() {
			...
			return this.activeStepIdx !== 0 ? this.activeStepIdx-- : false
		},
		reset() {
			// начать заново
		...
		nextOfFinish() {
			...
			return this.activeStepIdx < (this.steps.length -1) ? this.activeStepIdx++ : this.isFinish = true
		},
		setActive(idx) {
			// когда нажимаем на определенный шаг
			return !this.isFinish && this.activeStepIdx !== idx ? this.activeStepIdx = idx : false
		},
		kekw() {
			this.$refs.container.classList.add('animate__animated', 'animate__hinge')
		}
	},
	computed: {
